...
module.exports = {
    ...
    validateRule: async (req, res) => {
        try {
            ...
            let check = await checkObjectFields(rule, ['field', 'condition', 'condition_value']);
            if (!check.success) {
                ...
                switch (splittedField.length) {
                    case 1:
                        if (!data[splittedField[0]]) {
                            return apiResponse(res, 400, "error", `field ${splittedField[0]} is missing from data.`, null)
                        }
                        break;
                    case 2:
                        if (!data[splittedField[0]]) {
                        ...
                        if (!data[splittedField[0]][splittedField[1]]) {
                            return apiResponse(res, 400, "error", `field ${splittedField[1]} of ${splittedField[0]} is missing from data.`, null)
                        }
                        break;
                    default:
                        break;
    ...
    handleErrors: (req, res) => {
        return apiResponse(res, 404, "error", "Invalid Route", { error: "requested route not found" })
    }
};
